...
if (canvas && ctx) {
  ...
  class Particle {
    ...
    move(time) {
      ...
      const floatX = Math.cos(time * 0.00042 + this.phase) * 3.2 * this.z;
      const floatY = Math.sin(time * 0.00052 + this.phase) * 3.2 * this.z;
    ...
    draw() {
      const size = 0.95 + this.z * 1.7;

      ctx.beginPath();
      ctx.arc(this.x, this.y, size, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(242,217,142,${0.28 + this.z * 0.30})`;
      ctx.shadowBlur = 7 + this.z * 8;
      ctx.shadowColor = 'rgba(242,217,142,.18)';
      ctx.fill();
      ctx.shadowBlur = 0;
    }
  }
  ...
  function drawDepthGlow() {
    ...
    g1.addColorStop(0, 'rgba(242,217,142,.06)');
    g1.addColorStop(1, 'rgba(242,217,142,0)');

    const g2 = ctx.createRadialGradient(w * 0.84, h * 0.74, 0, w * 0.84, h * 0.74, 420);
    g2.addColorStop(0, 'rgba(94,234,212,.035)');
    g2.addColorStop(1, 'rgba(94,234,212,0)');

    const g3 = ctx.createRadialGradient(w * 0.52, h * 0.38, 0, w * 0.52, h * 0.38, 620);
    g3.addColorStop(0, 'rgba(255,255,255,.02)');
  ...
  function drawTravelingLight() {
    ...
    if (mouse.active) {
      px += (mouse.x - w / 2) * 0.014;
      py += (mouse.y - h / 2) * 0.014;
    }

    const gradient = ctx.createRadialGradient(px, py, 0, px, py, lightOrb.radius);
    gradient.addColorStop(0, 'rgba(255,255,255,.08)');
    gradient.addColorStop(0.18, 'rgba(242,217,142,.10)');
    gradient.addColorStop(0.42, 'rgba(94,234,212,.045)');
      ...
      for (let b = a + 1; b < particles.length; b++) {
        ...
        if (dist < LINK_DISTANCE) {
          ...
          if (lightDist < lightOrb.radius) {
            alpha += (1 - lightDist / lightOrb.radius) * 0.18;
          }

          if (mouse.active) {
            const mouseDist = Math.hypot(midX - mouse.x, midY - mouse.y);
            if (mouseDist < 170) {
              alpha += (1 - mouseDist / 170) * 0.08;
            }
          }
          ...
          ctx.strokeStyle = `rgba(242,217,142,${Math.min(alpha, 0.34)})`;
          ctx.lineWidth = 0.45 + ((pa.z + pb.z) / 2) * 0.42;
          ctx.shadowBlur = 6;
          ctx.shadowColor = 'rgba(242,217,142,.08)';
          ctx.stroke();
          ctx.shadowBlur = 0;
        }
      }
